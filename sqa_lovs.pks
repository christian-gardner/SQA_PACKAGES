...
SQL_STMT := 'SELECT PID, NVL(WORKING,''N'')AS WORKING, NVL(SQA_TD_REP,''NOT-ASSIGNED'') AS SQA_TD_REP, REVIEW_ID,  0 AS BATCH_NO, COMPLETED_DT, RK ';
...
------ latest fail
 ...
 LEFT JOIN ( SELECT LTRIM(LOANNUMBER,'0') AS LOANNUMBER, CLIENT,  REVIEWER AS BACKLOG  FROM SQA_ICC_BACKLOG  WHERE COMPLETED = 0 ) Q ON ( Q.LOANNUMBER = A.LOANNUMBER AND A.CLIENTCODE = Q.CLIENT);
...
SELECT R.LOANNUMBER,R.CLIENTCODE, R.DATA_SOURCE
 ...
 from inticc_daily_vw  where  COMPLETIONENTEREDDATE_FK  = (to_number(to_char(TODAY,'MMDDYYYY')) -1)
 UNION
 select  LOANNUMBER,"Client_Code" AS CLIENTCODE, 'FIPINT' AS DATA_SOURCE
 from fipint_daily_vw  where  COMPLETIONENTEREDDATE  = ( to_number(to_char(TODAY,'MMDDYYYY')) -1)
 UNION
